const getUserPosts = (req,res,connection)=>{
    ...
    const sql = `SELECT posts.id AS id ,author_id,posts.timestamp as timestamp,text,likes, users.nick AS nick FROM posts INNER JOIN users WHERE users.id = author_id AND author_id=${req.query.id} ORDER BY timestamp DESC LIMIT ${req.query.limit}`;
